"""Validate the isolated Gobierno Regional del Maule dry-run capture."""
...
REPORT_PATH = ROOT / "output" / "sources" / "gore_maule" / "report.md"
...
def _official_gore_maule_url(value: Any) -> bool:
...
def main() -> int:
    ...
    for index, item in enumerate(opportunities):
        ...
        if not _official_gore_maule_url(item.get("source_url")):
            errors.append(f"{label}.source_url debe ser oficial GORE Maule HTTP(S).")
        if not _official_gore_maule_url(item.get("listing_url")):
            errors.append(f"{label}.listing_url debe ser oficial GORE Maule HTTP(S).")
        if item.get("region") != "Maule" or item.get("commune") not in {None, "Talca"}:
            errors.append(f"{label} debe pertenecer a Maule; commune solo puede ser null o Talca.")
        ...
        if not isinstance(documents, list):
            ...
            for document in documents:
                if not isinstance(document, dict) or not document.get("name") or not _official_gore_maule_url(document.get("url")):
                    errors.append(f"{label}.document_urls contiene URL no oficial GORE Maule.")
    ...
    try:
        ...
        published_gore_maule = [
            item
            for item in public_opportunities
            if isinstance(item, dict)
            and (
                str(item.get("id") or "").startswith("gore-maule-")
                or item.get("source") == "GORE Maule"
            )
        ]
        if published_gore_maule:
            errors.append("GORE Maule debe permanecer aislado: se detectaron registros en public/data.")
