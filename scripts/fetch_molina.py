...
from radar.sources.molina import SOURCE_ID, fetch_molina_candidates


CATALOG_PATH = ROOT / "data" / "source_candidates.json"
OUTPUT_DIR = ROOT / "output" / "sources" / "molina"
...
def _write_report(candidate: dict[str, Any], diagnostics: dict[str, Any]) -> None:
    lines = [
        ...
        f"- Entradas revisadas: {diagnostics['listing_items_reviewed']}",
        f"- Publicaciones detectadas: {diagnostics['publications_detected']}",
        f"- Documentos detectados: {diagnostics['documents_detected']}",
        f"- Enlaces externos oficiales: {diagnostics['external_official_links']}",
        ...
        "## Alcance",
        "",
        "La captura consulta una sola página oficial configurada. No sigue enlaces ni",
        "descarga documentos. Los estados se asignan con reglas conservadoras y la",
        "salida no se publica en el dashboard.",
    ]
...
def main() -> int:
    try:
        ...
        opportunities, diagnostics = fetch_molina_candidates(candidate["discovery_url"])
    ...
    print("Dry-run Municipalidad de Molina")
    print("------------------------------")
    print(f"URL configurada: {candidate['discovery_url']}")
    print(f"Entradas revisadas: {diagnostics['listing_items_reviewed']}")
    ...
    print(f"Enlaces externos oficiales: {diagnostics['external_official_links']}")
