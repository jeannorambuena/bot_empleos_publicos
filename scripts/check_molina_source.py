...
OPPORTUNITIES_PATH = ROOT / "output" / "sources" / "molina" / "opportunities.json"
REPORT_PATH = ROOT / "output" / "sources" / "molina" / "report.md"
PUBLIC_OPPORTUNITIES_PATH = ROOT / "public" / "data" / "opportunities.json"
ALLOWED_STATUS = {"open_confirmed", "closed", "manual_review"}
DETAIL_URL_STATUS = {"document", "external_official", "listing_only"}
CONFIDENCE = {"high", "low"}
FIELDS = {
    "id",
    "source_id",
...
def _official_molina_url(value: Any) -> bool:
...
def _official_source_url(value: Any) -> bool:
    if not _valid_url(value):
        return False
    hostname = (urlparse(value).hostname or "").lower()
    return _official_molina_url(value) or hostname == "empleospublicos.cl" or hostname.endswith(".empleospublicos.cl")
...
def main() -> int:
    ...
    for index, item in enumerate(opportunities):
        ...
        if not _official_source_url(item.get("source_url")):
            errors.append(f"{label}.source_url debe ser oficial HTTP(S).")
        if not _official_molina_url(item.get("listing_url")):
            errors.append(f"{label}.listing_url debe ser oficial Molina HTTP(S).")
        if item.get("region") != "Maule" or item.get("commune") != "Molina":
            errors.append(f"{label} debe pertenecer a Maule / Molina.")
        ...
        if not isinstance(documents, list):
            ...
            for document in documents:
                if not isinstance(document, dict) or not document.get("name") or not _official_molina_url(document.get("url")):
                    errors.append(f"{label}.document_urls contiene URL no oficial Molina.")
    ...
    if len(ids) != len(set(ids)):
        errors.append("Los IDs Molina deben ser únicos.")
    ...
    try:
        ...
        published_molina = [
            ...
            and (
                str(item.get("id") or "").startswith("municipalidad-molina-")
                or item.get("source") == "Municipalidad de Molina"
            )
        ]
        if published_molina:
            errors.append("Molina debe permanecer aislado: se detectaron registros en public/data.")
